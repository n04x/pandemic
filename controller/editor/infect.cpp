...
auto infect::run(context &ctx, const args_type &args, ostream_type &out) const -> void {
	try {
		...
		auto city_infected_color = ctx.cities.get_color(
				city_infected_name);        // get the color of the city to infect
		ctx.cities.add_cube(city_infected_name, city_infected_color,
							cube_count);    // Add the cube to the infected city
		ctx.game.remove_cube_from_supply(city_infected_color, cube_count);            // Remove these cube from supply
		ctx.decks.add_to_top(discard_deck,
							 city_infected_name);                        // Put the city card in the discard deck
