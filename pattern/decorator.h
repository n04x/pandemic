...
struct decks_decorator : decks_model {
	...
	inline virtual auto get_card_view() const -> std::vector<handle> {
...
struct epidemic_decks_decorator : decks_decorator {
	...
	inline auto get_card_view() const -> std::vector<handle> override {
		...
		auto cards = decks_decorator::get_card_view();
